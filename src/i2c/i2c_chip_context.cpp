...
float i2cChipHealth(const ChipContext* ctx) {
    if (!ctx || !ctx->inUse || ctx->state == ChipState::STATE_LOCKED) return 0.0f;
    if (ctx->state == ChipState::STATE_ERROR) return 10.0f; // Salud mínima

    // Cálculo: Éxitos vs Errores totales (Escalado 0-100)
    if (ctx->totalReads == 0) return 100.0f;
    
    float errorRate = (float)ctx->totalErrors / (float)ctx->totalReads;
    float health = 100.0f * (1.0f - errorRate);
    
    // Penalización inmediata por errores consecutivos
    health -= (ctx->consecutiveErrors * 20.0f);
