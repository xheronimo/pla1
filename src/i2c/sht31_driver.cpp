...
{
    ChipContext* ctx = i2cGetChipContext(I2CDevice::SHT31, addr);
    if (!ctx) return false;
...
{
    ChipContext* ctx = i2cGetChipContext(I2CDevice::SHT31, s.address);
    if (!ctx) return false;

    uint32_t now = millis();

    // Guard común (ERROR, disabled, cooldown, warmup...)
