...
void drawModbusDiag() {
    ...
    display.setTextSize(1);
    display.setCursor(0,0);
    display.printf("BUS: %s", g_busStatusStr.c_str());
    
    // Barra de carga dinámica
    display.drawRect(0, 12, 128, 7, WHITE);
    int barW = (int)(g_modbusLoad * 1.28f);
    display.fillRect(0, 12, barW, 7, WHITE);
    
    display.setCursor(0, 25);
    display.printf("Rate: %u req/s", g_peticionesOk);
    
    if (g_lastFaultId != 0) {
        display.fillRect(0, 38, 128, 26, WHITE);
        display.setTextColor(BLACK);
        display.setCursor(2, 41);
        display.printf("FAIL ID: %d", g_lastFaultId);
        display.setCursor(2, 52);
        uint32_t sec = (millis() - g_lastFaultTime) / 1000;
        display.printf("Ago: %us", sec);
    }
    display.display();
}